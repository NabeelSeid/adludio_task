...
def make_recommendation():
    ...
    slice_parameters = {i: json_dict[i]
                        for i in json_dict if i != 'campaign_objective'}
    ...
    print('mmmma', matching_rows, file=sys.stdout)
    ...
    for i in ind_sort:
        ...
        game_version = REC_DATA.extract_game_version(game_id)[0]
        game_features = REC_DATA.extract_game_features(game_id=game_id)
        rec_score.append({'game_id': game_id, 'version': game_version,
                          'ctr': gm_kys_view[game_id]['click_through_rate'],
                          'e_r': gm_kys_view[game_id]['engagement_rate'],
